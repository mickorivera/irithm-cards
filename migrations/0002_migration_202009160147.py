...
def forward(old_orm, new_orm):
    ...
    return [
        # Check the field `usermodel.salt` does not contain null values
        # Check the field `usermodel.key` does not contain null values
    ]
...
def backward(old_orm, new_orm):
    usermodel = new_orm["usermodel"]
    return [
        # Apply default value '' to the field usermodel.password
        usermodel.update({usermodel.password: ""}).where(
            usermodel.password.is_null(True)
        ),
    ]
